...
Singletons, which are static storage duration objects, are constructed before the program's control flow enters `main`, and are not destructed until after it exits. [Static initialization order is not well-defined](https://isocpp.org/wiki/faq/ctors#static-init-order), and it is not even guaranteed the destructor will be called unless you can ensure that all exceptions are caught before the end of `main` *and* that the program cannot abort. Because of this, singletons are a very poorly suited pattern for [resource acquisition](https://en.cppreference.com/w/cpp/language/raii).
